...
rundeck_login_session() {
	...
	if ! rundeck_curl -c $COOKIES -b $COOKIES $url -o /dev/null
	then
		rerun_die 3 "curl request for cookie failed to $url (exit code: $?)"
    fi

    # Temporary file to store login page
    local -r curl_out=$(mktemp -t "login.XXXXX")

    # Submit the username and password to the login form.
	if ! http_code=$(rundeck_curl -c $COOKIES -b $COOKIES -w "%{http_code}" \
		-d j_username=$user -d j_password=$password \
		-X POST $loginurl -o $curl_out)
	then
