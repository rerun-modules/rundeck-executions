...
# Created: Wed Dec 23 07:32:13 PST 2015
#
#/ usage: rundeck-executions:delete [ --ids <>] [ --file <>]  --url <${RUNDECK_URL:-}> [ --username <${RUNDECK_USER:-}>] [ --password <${RUNDECK_PASSWORD:-}>] [ --apikey <${RUNDECK_APIKEY:-}>] 
...
rerun_options_parse() {
    ...
    do
        OPT="$1"
        case "$OPT" in
            --ids) rerun_option_check $# $1; IDS=$2 ; shift ;;
